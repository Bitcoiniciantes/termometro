...
export default function Home(){
 ...
 return <main>
  ...
  <div className="workspaceMain">
    ...
    <div className="watchDesk"><div className="watchTop"><div><span className="watchLabel">RADAR RÁPIDO</span><b>Seus ativos</b></div><span className="deskSource">{market?`Fonte: ${market.source}`:"Aguardando dados"}</span></div><div className="assetChips">{assets.map((asset,i)=><div key={asset} className={`assetChip ${ticker===asset?"selected":""}`}><button className="assetSelect" onClick={()=>selectAsset(asset)}><span className={`coin coin${i%6}`}>{asset.slice(0,1)}</span><b>{asset}</b><small>{asset==="MSTR"?"NASDAQ":"USDT"}</small>{ticker===asset&&<i>ATIVO</i>}</button>{!defaults.includes(asset)&&<button className="assetRemove" onClick={()=>removeAsset(asset)} aria-label={`Remover ${asset}`} title={`Remover ${asset}`}>×</button>}</div>)}</div></div>
  </div>
