...
<div className="biasBoard"><div className="biasHead"><span>RADAR LONG × BEAR</span><small>Viés comparativo em {period}</small></div><div className="biasList">{ranking.map((item,index)=>{const side=item.score>=20?"LONG":item.score<=-20?"BEAR":"NEUTRO";return <button key={item.asset} onClick={()=>selectAsset(item.asset)} className={`biasItem ${side.toLowerCase()}`}><span className="biasRank">{index+1}</span><b>{item.asset}</b><span className="biasSide">{side}</span><strong>{item.score>0?"+":""}{item.score}</strong><i><em style={{width:`${Math.abs(item.score)}%`}}/></i></button>})}</div><p>Viés técnico comparativo; não representa ordem de entrada.</p></div>
  ...
  <article className={`card thermo heroThermo thermoTone ${toneClass}`}><Title kicker="TERMÔMETRO DO ATIVO" title="Leitura consolidada" extra={<button className="info" title="Regras fixas, sem IA">i</button>}/><div className="scoreRing" style={{"--score":`${(score+100)*1.8}deg`} as React.CSSProperties}><div><b>{score>0?"+":""}{score}</b><span>DE 100</span></div></div><h3>{label}</h3><p>{score>=20?"Convergência positiva, com risco controlado.":score<=-20?"Pressão vendedora predominante; evite antecipar reversão.":"Sinais mistos: aguarde confirmação."}</p><div className="scale"><div className="scaleTrack"><i style={{left:`${(score+100)/2}%`}}/></div><div><span>-100<br/>Venda</span><span>0<br/>Neutro</span><span>+100<br/>Compra</span></div></div><div className="confidence"><span>Confiança da leitura</span><b>{confidence}%</b><div><i style={{width:`${confidence}%`}}/></div></div></article>
</section>
