...
| 16 | **Período padrão em 15M** (antes 1H). Vale para gráfico + análise (mesmo estado, igual ao preditivo). | `app/page.tsx` | `952b8df` |
| 17 | **NUPL voltou ao gráfico do BTC**: `NUPL_URL` apontada para `https://estudebitcoin.pages.dev/dados/nupl.json` (a antiga `bitcoiniciantes.github.io/...` retorna 404 e a barra vinha vazia). Componente, lib e CSS já eram idênticos ao preditivo. | `lib/api.ts` | `952b8df` |
