...
export default function Home(){
 ...
 useEffect(()=>{const controller=new AbortController();const intervals:Record<string,string>={"1H":"1h","4H":"4h","1D":"1d","1S":"1w"};const symbol=`${ticker}USDT`;const url=`https://data-api.binance.vision/api/v3/klines?symbol=${symbol}&interval=${intervals[period]||"1d"}&limit=120`;fetch(url,{signal:controller.signal}).then(async r=>{const body=await r.json();if(!r.ok)throw new Error(body.msg||"Ativo não disponível na Binance");const candles:Candle[]=body.map((row:(string|number)[])=>({time:Number(row[0]),open:Number(row[1]),high:Number(row[2]),low:Number(row[3]),close:Number(row[4]),volume:Number(row[5])}));setMarket({asset:ticker,pair:`${ticker}/USDT`,source:"Binance Public Market Data",updatedAt:Date.now(),candles})}).catch(e=>{if(e.name!=="AbortError"){setMarket(null);setMarketError(ticker==="MSTR"?"MSTR ainda sem fonte gratuita":"Ativo indisponível ou bloqueado pela fonte")}}).finally(()=>setLoading(false));return()=>controller.abort()},[ticker,period]);
 ...
 const fmt=(value:number|undefined)=>value?`USDT ${value.toLocaleString("pt-BR",{minimumFractionDigits:value>=100?2:3,maximumFractionDigits:value>=100?2:4})}`:"—";
 ...
 return <main>
  ...
  <div className="workspaceMain">
    <div className="deskTop">
      ...
      <div className="periods">{["1H","4H","1D","1S"].map(p=><button key={p} onClick={()=>{setLoading(true);setMarketError("");setPeriod(p)}} className={period===p?"active":""}>{p}</button>)}</div>
    </div>
    <div className="compactControls">
      <form onSubmit={e=>{e.preventDefault();if(query.trim())selectAsset(query.trim().toUpperCase())}}><label>BUSCAR ATIVO</label><div><input aria-label="Código do ativo" value={query} onChange={e=>setQuery(e.target.value)} placeholder="BTC"/><button>Analisar →</button></div></form>
      <div className="addAsset"><input aria-label="Novo ativo" value={newAsset} onChange={e=>setNewAsset(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addAsset()}}} placeholder="Adicionar código..."/><button onClick={addAsset}>+ Incluir</button></div>
    </div>
    <div className="watchDesk"><div className="watchTop"><div><span className="watchLabel">RADAR RÁPIDO</span><b>Seus ativos</b></div><span className="deskSource">{market?`Fonte: ${market.source}`:"Aguardando dados"}</span></div><div className="assetChips">{assets.map((asset,i)=><button key={asset} onClick={()=>selectAsset(asset)} className={ticker===asset?"selected":""}><span className={`coin coin${i%6}`}>{asset.slice(0,1)}</span><b>{asset}</b><small>{asset==="MSTR"?"NASDAQ":"USDT"}</small>{ticker===asset&&<i>ATIVO</i>}</button>)}</div></div>
  </div>
  <article className="card thermo heroThermo"><Title kicker="TERMÔMETRO DO ATIVO" title="Leitura consolidada" extra={<button className="info" title="Regras fixas, sem IA">i</button>}/><div className="scoreRing" style={{"--score":`${(score+100)*1.8}deg`} as React.CSSProperties}><div><b>{score>0?"+":""}{score}</b><span>DE 100</span></div></div><h3>{label}</h3><p>{score>=20?"Convergência positiva, com risco controlado.":"Sinais mistos: aguarde confirmação."}</p><div className="scale"><div className="scaleTrack"><i style={{left:`${(score+100)/2}%`}}/></div><div><span>-100<br/>Venda</span><span>0<br/>Neutro</span><span>+100<br/>Compra</span></div></div><div className="confidence"><span>Confiança da leitura</span><b>{confidence}%</b><div><i style={{width:`${confidence}%`}}/></div></div></article>
</section>
  <section className="dashboard"><div className="grid">
    <article className="card chart"><Title kicker="ESTRUTURA DE PREÇO" title={patternTitle} extra={<span className="badge">CONFIANÇA {confidence}%</span>}/><div className="priceChart"><div className="resistance"><span>RESISTÊNCIA {fmt(resistance)}</span></div><div className="support"><span>SUPORTE {fmt(support)}</span></div><div className="candles">{candleHeights.map((h,i)=><i key={i} className={i%4===0||i%7===0?"red":"green"} style={{height:`${h}%`,transform:`translateY(${i%3*5}px)`}}><b/></i>)}</div></div><div className="chartFoot"><span><i className="dot greenDot"/>MM20</span><span><i className="dot blueDot"/>MM50</span><span>Volume <b>{volumeRatio?`${volumeRatio.toFixed(2)}× média`:"—"}</b></span></div></article>
